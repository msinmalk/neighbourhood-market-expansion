...
def forecast_financials(df, assumptions, base_year, forecast_years):
    ...
    if base_year in forecast_df.columns:
        calculate_cagr(forecast_df, base_year, forecast_years)  # Calculate CAGR and add as a column
    else:
        calculate_total_cagr(df, forecast_df, base_year, forecast_years)
...
def calculate_total_cagr(df, forecast_df, base_year, forecast_years):
    """
    Calculates the compound annual growth rate (CAGR) for forecasted data including the current year.
...
def linear_regression_forecast(df, forecast_years, transform_if_needed=True):
    ...
    if len(forecast_years) == 1:
        historical_cagr_values = calculate_total_cagr(df, historical_regression_forecast, base_year, forecast_years)
    else:
...
def categorize_and_map_metrics(financial_df, metrics_patterns=None):
    ...
    for category, metrics in categorized_metrics.items():
        if category == 'assets':
            ...
            base_metric = "Total shareholders' funds and liabilities"
